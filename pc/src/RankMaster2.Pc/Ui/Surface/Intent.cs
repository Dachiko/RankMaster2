...
/// <summary>
/// The eleven intents of plan § 1.1: everything a key or a click can mean on either screen.
/// <see cref="None"/> is "swallow the key, do nothing" — the compare screen's catch-all so
/// Avalonia's own key handling never runs (plan § 3.7).
/// </summary>
public enum Intent
{
    None = 0,
...
public static class Intents
{
    ...
    /// <summary>True for the seven intents that name one particular pair action (the ones the gate
    ...
    public static bool IsPairAction(this Intent intent) => intent is
        Intent.VoteLeft or Intent.VoteRight or Intent.Skip or
    ...
    /// <summary>The side a pair action is "about", for pane-readiness (plan § 3.2). Null for Skip,
