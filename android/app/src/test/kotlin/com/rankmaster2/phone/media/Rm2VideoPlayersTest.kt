...
class Rm2VideoPlayersTest {
    ...
    // -- a rebuilt player knows nothing; the pane is what waits for it ------------------------

    @Test
    fun `a player rebuilt after a release starts with its shape unknown, whatever the last one knew`() {
        // The slot briefly seeded a rebuilt player with the shape its predecessor had reported for
        // the same file (2026-09-17), so a pane resuming from under the full-screen viewer would
        // not flash the wrong shape. That was the wrong end to fix it from: the pane now shows
        // nothing until its own decoder has reported (VideoPaneLayoutTest), so "unknown" is the
        // honest and harmless starting point, and the only thing that ever tells a pane a picture's
        // shape is the decoder drawing that picture.
        val ref = MediaFixtures.video("a.mp4")
        ...
        assertNull(slot.sync(ref, playing = false))
        val resumed = slot.sync(ref, playing = true)
        requireNotNull(resumed)

        assertNull("a fresh player's shape is unknown until its decoder says otherwise", resumed.aspectRatio.value)
    }
}
